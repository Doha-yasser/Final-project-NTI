<form method="GET" action="{{ route('courses.index') }}" class="my-course-filter-wrap d-flex align-items-center pt-2">
    ...
    <div class="my-course-filter-item my-course-sort-by-content mr-3">
        <span class="fs-14 font-weight-semi-bold">{{__("site.Sort by") }}</span>
        <div class="select-container w-100 pt-2">
            <select name="sort" class="select-container-select">
                <option value="new" @if(request('sort')=='new' ) selected @endif>
                    <span class="success-message">{{__("site.New") }}</option>
                <option value="old" @if(request('sort')=='old' ) selected @endif>
                    <span class="success-message">{{__("site.Old") }}</option>
            </select>
        </div>
    </div>
    ...
    <div class="my-course-filter-item my-course-search-content">
        <span class="fs-14 font-weight-semi-bold">{{__("site.Search") }}</span>
